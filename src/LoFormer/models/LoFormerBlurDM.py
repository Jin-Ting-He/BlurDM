...
from LoFormer.models.archs.arch_util import *
from LoFormer.models.archs.dct_util import *
from LoFormer.models.archs.up_down import *
...
class Attention(nn.Module):
    def __init__(self, dim, prior_exist, num_heads, bias, window_size=8, grid_size=8, window_size_dct=9,
                 qk_norm=True, proj_out=True, temp_div=True, norm_dim=-1, cs='channel', padding_mode='zeros'):
        super().__init__()
        self.prior_exist = prior_exist
        if self.prior_exist:
...
class TransformerBlock(nn.Module):
    def __init__(self, dim=32,
        ...
        self.prior_exist = prior_exist
        # if self.prior_exist:
        #     self.kv1 = nn.Linear(256, 2 * dim, bias=bias)
        #     self.kv2 = nn.Linear(256, 2 * dim, bias=bias)
        ...
        self.attn = nn.Sequential(
            Attention(dim, prior_exist, num_heads, bias, window_size_dct=window_size_dct,
    ...
    def forward(self, inp):
        ...
        if self.prior_exist:
            x = inp[0]
            prior = inp[1]
            # print("Test", prior.shape)
            if 'LN_DCT' in self.cs:
                x_dct = self.dct(self.norm1(x))
                x_attn = self.attn([x_dct, prior])
                x = self.idct(x_attn) + x
            else:
                x_dct = self.dct(x)
                x_attn = self.attn([self.norm1(x_dct), prior])
                x_dct = x_dct + x_attn
                x = self.idct(x_dct)
        else:
            x = inp
...
class TransformerBlock_2b(nn.Module):
    def __init__(self, dim=32,
        ...
        self.trans1 = TransformerBlock(dim, prior_exist, num_heads, ffn_expansion_factor,
        ...
        self.trans2 = TransformerBlock(dim, prior_exist, num_heads, ffn_expansion_factor,
    ...
    def forward(self, inp):
        if self.prior_exist:
            x = inp[0]
            prior = inp[1]
            x = self.trans1([x, prior])
            x = self.trans2([x, prior])
            return [x, prior]
        else:
            x = inp
            x = self.trans1(x)
...
class LoFormerPrior(nn.Module):
    def __init__(self,
        ...
        super(LoFormerPrior, self).__init__()
        ...
        for j in range(len(enc_blk_nums)):
            ...
            self.encoders.append(
                nn.Sequential(
                    *[TransformerBlockx(dim=chan, prior_exist=False, num_heads=heads_enc[j], ffn_expansion_factor=ffn_expansion_factor,
        ...
        self.middle_blks = \
            nn.Sequential(
                *[TransformerBlockx(dim=chan, prior_exist=False, num_heads=heads_mid, ffn_expansion_factor=ffn_expansion_factor,
                                    bias=bias, LayerNorm_type=LayerNorm_type,
        ...
        for j in range(len(dec_blk_nums)):
            ...
            self.decoders.append(
                nn.Sequential(
                    *[TransformerBlockx(dim=chan, prior_exist=True, num_heads=heads_dec[j], ffn_expansion_factor=ffn_expansion_factor,
                                        bias=bias, LayerNorm_type=LayerNorm_type,
    ...
    def forward(self, inp_img, prior, file_name=None):
        # print(inp_img.shape)
...
#     net = LoFormerPrior(dim=dim, enc_blk_nums=enc_blk_nums, middle_blk_num=middle_blk_num,dec_blk_nums=dec_blk_nums,
